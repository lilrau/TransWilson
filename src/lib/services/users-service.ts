...
export const getUserByUsername = unstable_cache(
  async (username: string) => {
    try {
      ...
      Logger.error("users-service", "Unexpected error while fetching user by username", {
        error,
        username,
      })
      throw error
...
export const createUser = async (data: UserData) => {
  try {
    Logger.info("users-service", "Creating new user", {
      userData: { ...data, user_senha: undefined },
    })
...
export const updateUser = async (id: number, data: Partial<UserData>) => {
  try {
    Logger.info("users-service", "Updating user", {
      id,
      userData: { ...data, user_senha: undefined },
    })
    ...
    const result = await supabase().from("users").update(updatedData).eq("id", id).select()
