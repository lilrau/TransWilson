...
export const getAllMotorista = unstable_cache(
  async (): Promise<MotoristaData[]> => {
    try {
      ...
      Logger.info("motorista-service", "Successfully fetched all motoristas", {
        count: data.length,
      })
      return data
    } catch (error) {
...
export const getMotorista = unstable_cache(
  async (id: number): Promise<MotoristaData> => {
    try {
      ...
      Logger.error("motorista-service", "Unexpected error while fetching motorista by id", {
        error,
        id,
      })
      throw error
...
export const createMotorista = async (data: Partial<MotoristaData>) => {
  try {
    ...
    Logger.info("motorista-service", "Successfully created motorista", {
      motoristaId: result.data[0].id,
    })
    return result.data
  } catch (error) {
  ...
  try {
    ...
    if (result.error?.code === "23503") {
      const error = new Error(
        "Não é possível excluir o motorista porque há pedidos associados a ele."
      )
      Logger.error("motorista-service", "Failed to delete motorista - associated orders exist", {
        error,
        id,
      })
      throw error
